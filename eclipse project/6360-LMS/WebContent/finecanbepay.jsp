...
<html>
  <head>
    ...
    <meta name="description" content="Fine Pay Page"/>
  ...
  <body>
	...
	<%-- PROCESS FINE CAN BE PAY --%>
	<%! String mysqlpassword = "123"; %>
	<%
		// use the Library_Management_System library
		String sqlcmd0="USE Library_Management_System;";
		// 
		String cardNo=request.getParameter("fncanbepaycardno");
		String sqlcmd1 = "SELECT BL.Card_no, SUM(F.fine_amt) AS Total_fine_canbepay FROM BOOK_LOANS AS BL, FINES AS F WHERE BL.Loan_id=F.Loan_id AND F.paid='0' AND (BL.Date_in IS NOT NULL) AND Card_no='" + cardNo + "';";	
	...
	<!-- FINE CAN BE PAY RESULT
	================================================== -->
	<div class="container">
	  <div class="page-header text-primary"><h4 class="text-center">Result:</h4></div>
	  <%
	  	if (result1.next() && result1.getString("Total_fine_canbepay") != null) {	// there are fine can be pay exist
	  %>
	  	<table class="table table-striped">
	  	  <thead>
		    <tr>
		      <th>Card ID</th>
		      <th>Total Fine Can be Pay</th>
		    </tr>
		  </thead>
		  <tbody>
		    <tr>
		      <td><%=result1.getString("Card_no") %></td>
		      <td><%=result1.getString("Total_fine_canbepay") %></td>
		    </tr>
		  </tbody>
	  	</table> 		
	  <%	
			...
			<div class="alert alert-success" role="alert">
	        	<strong>HELLO!</strong> There isn't any fine can be pay at this moment.
	      	</div>
	  <%}
